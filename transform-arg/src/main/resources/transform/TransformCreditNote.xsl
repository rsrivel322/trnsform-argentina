...
<!-- 		</xsl:element> -->
	...
	<!--GLOBAL-->
	<xsl:variable name="TipoDoc" select="node()/cbc:CreditNoteTypeCode"/>
	<xsl:variable name="Electronica" select="'61'"/>
	<xsl:variable name="Exportacion" select="'112'"/>
	<xsl:template match="/">
	<!--SETDTE-->
					...
					<xsl:element name="Encabezado">
						...
						<xsl:element name="IdDoc">
							<xsl:element name="TipoDTE">
								<xsl:value-of select="node()/cbc:CreditNoteTypeCode"/>
							</xsl:element>
...
<!-- 								</xsl:if> -->
					...
					<!--DETALLE-->
					<xsl:for-each select="node()/cac:CreditNoteLine">
